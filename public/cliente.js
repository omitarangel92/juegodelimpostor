// public/cliente.js (CÓDIGO FINAL CORREGIDO PARA RENDER/RAILWAY)

// --- CRÍTICO: Esperar a que el DOM esté cargado para evitar el error 'Cannot read properties of null' ---
document.addEventListener('DOMContentLoaded', (event) => {

    // 1. CONEXIÓN SOCKET.IO (Ajuste para Render)
    const socket = io({ 
        transports: ['websocket', 'polling'] // Forzar el uso de transports compatibles con Render
    }); 
    ...
    let configuracionActual = { tema: 'Animales 🐾', tiempoRondaSegundos: 60, numRondas: 99, incluirAgenteDoble: false }; 
    ...
    function actualizarListaJugadores(jugadores) {
        ...
        const renderJugadores = (listaElement, isHostView) => {
            if(!listaElement) return; // Protección extra
    ...
    function actualizarBotonInicioJuego() {
        ...
        if (esHost) {
            ...
            } else if (jugadoresActuales.length > MAX_JUGADORES) {
                btnIniciar.disabled = true;
                btnIniciar.textContent = `¡Máximo ${MAX_JUGADORES} jugadores! (Expulsa jugadores)`;
            }
            else {
    ...
    // ESTA ERA LA SECCIÓN QUE FALLABA, AHORA ESTÁ SEGURA DENTRO DEL DOMContentLoaded
    ...
    document.getElementById('btn-iniciar-juego').addEventListener('click', () => {
        if (jugadoresActuales.length >= MIN_JUGADORES_CLIENTE && jugadoresActuales.length <= MAX_JUGADORES) {
            socket.emit('iniciarJuego', codigoSalaActual);
        } else {
            alert(`Se necesitan al menos ${MIN_JUGADORES_CLIENTE} y máximo ${MAX_JUGADORES} jugadores.`);
        }
    });
    ...
    // Exportar funciones para uso en onclick (si el código no está modularizado)
    ...
    window.seleccionarVoto = function(jugadorId) {
        ...
        if (jugadorId !== 'none') {
            ...
            document.getElementById(`opcion-none`).classList.add('seleccionado');
        }
    }
    ...
    socket.on('juegoIniciado', (data) => {
        ...
        // Ocultar botones de Host si no lo es 
    ...
    function renderOpcionesVoto(jugadoresActivos) {
        ...
        if (!contenedorVotos) return;
        contenedorVotos.innerHTML = '';

        jugadoresActivos.forEach(j => {
            // Solo se puede votar a jugadores activos que no seas tú (a menos que estés eliminado)
        ...
        if (!jugadoresActuales.find(p => p.id === socket.id)?.eliminado) {
...
}); // CIERRE DEL DOMContentLoaded: ¡IMPORTANTE!