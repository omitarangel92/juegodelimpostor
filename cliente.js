...
document.getElementById('btn-reglas').onclick = (e) => {
    ...
    const reglasTexto = `
        ...
        <ul>
            <li>Cada partida, la IA inventa una <b>categoría</b> y elige <b>una sola palabra secreta</b> dentro de ella.</li>
        ...
        <ul>
            ...
            <li style="margin-top:10px"><b style="color:var(--color-red)">🟥 IMPOSTOR:</b> Solo ve la categoría, NUNCA la palabra. Misión: fingir que la sabe. <b>Poder especial 🌟:</b> puede tocar el 🎤 para adivinar la palabra y ganar al instante.</li>
            <li style="margin-top:10px"><b style="color:var(--color-orange)">⬜ AGENTE BLANCO:</b> Conoce la categoría pero NO la palabra secreta y tampoco es Impostor. Misión: sobrevivir fingiendo que sabe algo.</li>
        </ul>
        <h3 style="color:var(--color-secondary)">🗣️ 5. Fases de la Partida</h3>
        <ol>
            <li><b>Revelación:</b> Cada quien ve su rol y su palabra en privado.</li>
            <li><b>Discusión:</b> Turnos de 15 segundos. Una pista corta sin decir la palabra.</li>
            <li><b>Votación:</b> Toca al jugador que más sospeches. Puedes cambiar tu voto hasta que el Capitán la cierre.</li>
            <li><b>Veredicto:</b> Se revela a quién expulsaron y qué rol tenía.</li>
        </ol>
        <h3 style="color:var(--color-primary)">🔁 6. Rondas múltiples</h3>
        <ul>
            <li><b>Empate:</b> nadie es expulsado. Nueva ronda automática en 5 segundos.</li>
            <li><b>Expulsan Tripulante inocente:</b> queda 🔒 eliminado permanentemente. Nueva ronda con sobrevivientes.</li>
            <li><b>Expulsan al Agente Blanco:</b> el juego termina al instante (¡él gana!).</li>
            <li><b>Expulsan a un Impostor:</b> si quedan más, la partida sigue. Si era el último, ganan los Tripulantes.</li>
            <li><b>Dos empates seguidos:</b> los Impostores ganan.</li>
            <li><b>Superioridad numérica:</b> si los Impostores igualan o superan a los Tripulantes, ganan automáticamente.</li>
        </ul>
        <h3 style="color:var(--color-secondary)">🏆 7. Cómo se gana</h3>
        <ul>
            <li><b style="color:var(--color-green)">Tripulantes ganan</b> si expulsan a TODOS los Impostores.</li>
            <li><b style="color:var(--color-red)">Impostores ganan</b> si: igualan en número, hay dos empates seguidos, o uno adivina la palabra.</li>
        ...
        <ul>
            ...
            <li><b>La IA</b> inventa categoría y palabra, se burla entre rondas, y al final redacta un <b>mensaje personalizado para CADA jugador</b>.</li>
        </ul>
    `;
...
async function generarVeredictosIA(situacion, dificultad, roster) {
    ...
    const systemPrompt = dificultad === 'adultos'
        ? `Eres el juez más cruel y gracioso de un juego de fiesta para adultos. Tu especialidad es humillar a los perdedores con frases tan certeras y específicas a su rol que se van a reír mientras se sienten mal consigo mismos. A los ganadores también los tratas con sarcasmo: los felicitas pero con una pulla que les baja el ego. REGLAS: sin groserías, sin insultos personales reales, sin contenido sexual. El humor es situacional, específico al rol que jugó cada persona. Respondes ÚNICAMENTE con JSON válido.`
        : `Eres el juez más pícaro y gracioso de un juego familiar para todas las edades. A los perdedores les das frases que los hacen reír de vergüenza ajena — algo que los deja en ridículo de forma divertida y sin crueldad real. A los ganadores los felicitas con un halago que tiene trampa — algo bueno pero con una pulla suave al final. Todo apto para niños de 10 años en adelante. Respondes ÚNICAMENTE con JSON válido.`;

    const instruccionesPorRol = dificultad === 'adultos' ? `
INSTRUCCIONES POR ROL (adultos, sin groserías):
- IMPOSTOR PERDIÓ: humíllalo por ser tan malo mintiendo. Ej: "actuaste tan sospechoso que hasta el Wi-Fi de la sala te quería expulsar."
- IMPOSTOR GANÓ: felicítalo con sarcasmo. Ej: "ganaste, pero todos sabemos que fue de chiripa — la próxima no te salvas."
- TRIPULANTE PERDIÓ: búrlate de que votó mal. Ej: "tenías la respuesta en la cara del impostor y aun así votaste al inocente. Nivel de ceguera impresionante."
- TRIPULANTE GANÓ: halago con trampa. Ej: "bien jugado, aunque admitamos que dudaste tres veces antes de votar bien."
- AGENTE BLANCO PERDIÓ: búrlate de que no confundió a nadie. Ej: "se supone que debías sembrar dudas, pero lo único que sembraste fue lástima."
- AGENTE BLANCO GANÓ: sarcasmo. Ej: "ganaste siendo el más inútil de la sala — eso sí es un talento especial."` : `
INSTRUCCIONES POR ROL (familiar, apto para niños):
- IMPOSTOR PERDIÓ: Ej: "te descubrieron tan rápido que ni el perro de la casa te hubiera creído."
- IMPOSTOR GANÓ: Ej: "¡ganaste! aunque todos sabemos que fue porque los demás son muy despistados."
- TRIPULANTE PERDIÓ: Ej: "tenías toda la información y aun así votaste mal — eso tiene mérito al revés."
- TRIPULANTE GANÓ: Ej: "¡bien! aunque tardaste tanto en decidir que casi se nos duerme el impostor esperando."
- AGENTE BLANCO PERDIÓ: Ej: "tu misión era confundir a todos y lo que lograste fue confundirte a ti mismo."
- AGENTE BLANCO GANÓ: Ej: "ganaste siendo el más misterioso — o el más callado, que a veces es lo mismo."`;

    const promptEnvio = `Situación final: "${situacion}"
...
- Cada frase DEBE ser específica al rol o lo que hizo ese jugador — nada genérico.
- Las frases de perdedores deben hacer reír AL LEERLAS.
- Las frases de ganadores deben tener un halago real seguido de una pulla.
- NUNCA repitas la misma estructura para dos jugadores.
- Sin groserías ni insultos personales reales.

JSON con ids exactos como llaves:
...
async function generarBurlaIntermedia(situacion, dificultad) {
    const systemPrompt = dificultad === 'adultos'
        ? `Eres un comentarista deportivo que narra el desastre de una tripulación adulta que acaba de cagar una votación. Tu comentario es cruel, gracioso y específico. Sin groserías. Máximo 30 palabras. Solo JSON.`
        : `Eres el narrador burlón de un juego familiar que acaba de ver a la tripulación votar pésimo. Tu comentario es divertido, juguetón y específico. Apto para niños. Máximo 30 palabras. Solo JSON.`;

    const promptEnvio = `Lo que acaba de pasar: "${situacion}"\n\nLa partida continúa. Escribe UNA frase que: (1) se burle específicamente de lo que pasó, (2) sea tan precisa que duela y haga reír, (3) anticipe que la próxima ronda puede ser peor.\n\nJSON: {"mensaje":"..."}`;
    ...
    try {
        ...
        const fallback = [
            ...
            "La tripulación expulsó al inocente con una convicción que da vergüenza ajena.",
            "Nadie se puso de acuerdo. Los impostores tomaron nota y piden otra ronda.",
            "Ese nivel de confusión solo se logra con mucho esfuerzo. O ninguno.",
